...
export const BRAZILIAN_STATES: readonly BrazilianStateInfo[] = [
...
const stateMap = new Map(BRAZILIAN_STATES.map((state) => [state.uf, state]));

export function getBrazilianStateInfo(uf: string) {
  return stateMap.get(uf.toUpperCase()) ?? null;
}

export function isValidBrazilianState(uf: string) {
  return stateMap.has(uf.toUpperCase());
}
